...
suite("TekisasuObjectPromise", () => {
	...
	test("resolves successfully with a valid TekisasuObject", async () => {
		const tekisasuObject: TekisasuObject = {
			tekisasu_id: BigInt(1),
			type: "TestType",
			sub_values: [],
		};

		const promise = new TekisasuObjectPromise();
		setTimeout(() => promise.resolve(tekisasuObject), 10);
		clock.tick(10); // Fast-forward time
		await expect(promise.promise).to.eventually.equal(tekisasuObject);
	});
	...
	test("rejects due to timeout", async () => {
		const promise = new TekisasuObjectPromise(50);
		clock.tick(50); // Fast-forward time
		await expect(promise.promise).to.be.rejectedWith("TekisasuObjectPromise timed out");
	});

	test("does not reject if resolved before timeout", async () => {
		const tekisasuObject: TekisasuObject = {
			tekisasu_id: BigInt(2),
			type: "AnotherTestType",
			sub_values: [],
		};

		const promise = new TekisasuObjectPromise(100);
		setTimeout(() => promise.resolve(tekisasuObject), 10);
		clock.tick(10); // Fast-forward time
		await expect(promise.promise).to.eventually.equal(tekisasuObject);
	});
	...
	test("clears timeout when resolved", async () => {
		const promise = new TekisasuObjectPromise(1000);
		promise.resolve({ tekisasu_id: BigInt(3), type: "ResolvedType", sub_values: [] });
